...
export async function GET(request, { params }) {
  ...
  const upstreamRes = await upstreamFetch(`/documents/${id}`, auth)
  if (upstreamRes.status === 401) return NextResponse.json({ detail: 'Unauthorized' }, { status: 401 })
  if (!upstreamRes.ok) {
    const data = await upstreamRes.json().catch(() => ({}))
    return NextResponse.json({ detail: data?.detail || 'Failed to fetch document' }, { status: upstreamRes.status || 500 })
  }

  const bytes = await upstreamRes.arrayBuffer()
  const contentType = upstreamRes.headers.get('content-type') || 'application/octet-stream'
  const contentLength = upstreamRes.headers.get('content-length')
  const contentDisposition = upstreamRes.headers.get('content-disposition') || `attachment; filename="document-${id}"`
